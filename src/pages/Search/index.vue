...
export default {
  ...
  // 组件挂载完毕之前执行一次【先与mounted之前】
  beforeMount() {
    // 在发送请求之前，把接口需要传递参数，进行整理（在给服务器发请求之前，把参数整理好，服务器就会返回查询的数据）
  ...
  methods: {
    ...
    removeCategoryName() {
      ...
      this.getData()
      // 地址栏也需要更改：进行路由跳转(仙子的路由跳转只是跳转到自己这里)
  ...
  watch: {
    ...
    $route(newValue, oldValue) {
      ...
      // 再次发起ajax请求
      this.getData()
      // 每一次请求完毕，应该把相应的1、2、3级分类id置为空，让他们接收下一次的相应1、2、3id
