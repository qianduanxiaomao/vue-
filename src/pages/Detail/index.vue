          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                ...
                把你购买的产品的信息通过请求的形式通知路由器，服务器进行相应的存储 -->
  ...
  methods: {
    ...
    async addShopcar() {
      ...
      this.$store.dispatch('addOrUpdateShopCart', {skuId: this.$route.params.skuid,skuNum: this.skuNum})
        上面这行代码说白了：调用Vuex仓库中的 addOrUpdateShopCart函数，这个方法加上asyc，返回一定是一个Promise
        要么成功|要么失败
      */
      try {
        // 成功
        ...
        // 一些简单的数据skuNum,通常query形式给路由组件传递过去
        // 产品信息的数据【比较复杂：skuInfo】，通常会话存储（不持久化，会话结束数据在消失）
        // 本地存储|会话存储,一般存储的是字符串
